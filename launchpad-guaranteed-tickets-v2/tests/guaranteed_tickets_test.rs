...
use dharitri_sc::types::{
    RewaOrDcdtTokenIdentifier, MultiValueEncoded, MultiValueEncodedCounted,
};
...
#[test]
fn remove_from_blacklist_underflow_and_empty_storage_test() {
    ...
    // Remove first participant from blacklist
    lp_setup
        .b_mock
        .execute_tx(
            &lp_setup.owner_address,
    ...
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[0], &rust_biguint!(2 * TICKET_COST));
    lp_setup.b_mock.check_dcdt_balance(
        &participants[0],
        LAUNCHPAD_TOKEN_ID,
    ...
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[1], &rust_biguint!(2 * TICKET_COST));
    lp_setup.b_mock.check_dcdt_balance(
        &participants[1],
        LAUNCHPAD_TOKEN_ID,
    ...
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[3], &rust_biguint!(0));
    lp_setup.b_mock.check_dcdt_balance(
        &participants[3],
        LAUNCHPAD_TOKEN_ID,
    ...
    let expected_owner_rewa = TICKET_COST * nr_winning_tickets as u64; // 6 winning tickets

    lp_setup
        .b_mock
        .check_rewa_balance(&lp_setup.owner_address, &rust_biguint!(expected_owner_rewa));
    lp_setup.b_mock.check_dcdt_balance(
        &lp_setup.owner_address,
        LAUNCHPAD_TOKEN_ID,
...
fn multiple_guaranteed_tickets_test() {
    ...
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[0], &rust_biguint!(3 * TICKET_COST));
    lp_setup
        .b_mock
        .check_dcdt_balance(&participants[0], LAUNCHPAD_TOKEN_ID, &rust_biguint!(0));

    // Second user: 2 tickets, no guarantee, no winning tickets
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[1], &rust_biguint!(3 * TICKET_COST));
    lp_setup
        .b_mock
        .check_dcdt_balance(&participants[1], LAUNCHPAD_TOKEN_ID, &rust_biguint!(0));
    ...
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[3], &rust_biguint!(TICKET_COST));
    lp_setup.b_mock.check_dcdt_balance(
        &participants[3],
        LAUNCHPAD_TOKEN_ID,
        &rust_biguint!(2 * LAUNCHPAD_TOKENS_PER_TICKET),
    ...
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[4], &rust_biguint!(2 * TICKET_COST));
    lp_setup.b_mock.check_dcdt_balance(
        &participants[4],
        LAUNCHPAD_TOKEN_ID,
    ...
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[5], &rust_biguint!(7 * TICKET_COST));
    lp_setup.b_mock.check_dcdt_balance(
        &participants[5],
        LAUNCHPAD_TOKEN_ID,
        &rust_biguint!(2 * LAUNCHPAD_TOKENS_PER_TICKET),
    ...
    // Check owner's balances
    let expected_owner_rewa = TICKET_COST * nr_winning_tickets as u64; // 9 winning tickets
...
#[test]
fn fair_chance_allocation_and_ticket_redistribution_test() {
    let nr_winning_tickets = 9;
    let mut lp_setup = LaunchpadSetup::new(
        nr_winning_tickets,
        launchpad_guaranteed_tickets_v2::contract_obj,
    ...
    lp_setup
        ...
        .execute_tx(
            ...
            |sc| {
                ...
                let mut guaranteed_tickets_info = MultiValueEncodedCounted::new();
                guaranteed_tickets_info.push((1, 6).into());
                guaranteed_tickets_info.push((2, 4).into());
                guaranteed_tickets_info.push((3, 3).into());
                args.push(
                    (
                        managed_address!(&new_participant),
    ...
    lp_setup.confirm(&participants[1], 2).assert_ok();
    lp_setup.confirm(&participants[2], 3).assert_ok();
    lp_setup.confirm(&participants[3], 4).assert_ok();
    ...
    // Check balances
    // First user: 1 ticket, no guarantee, no winning tickets
    let winning_tickets_user1 = 0u64;
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[0], &rust_biguint!(3 * TICKET_COST));
    lp_setup.b_mock.check_dcdt_balance(
    ...
    // Second user: 2 tickets, no guarantee, 2 winning tickets
    let winning_tickets_user2 = 2;
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[1], &rust_biguint!(TICKET_COST));
    lp_setup.b_mock.check_dcdt_balance(
        &participants[1],
        LAUNCHPAD_TOKEN_ID,
        &rust_biguint!(winning_tickets_user2 * LAUNCHPAD_TOKENS_PER_TICKET),
    ...
    // Third user: 3 tickets, 1 guaranteed, 3 winning ticket
    let winning_tickets_user3 = 3;
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[2], &rust_biguint!(0));
    lp_setup.b_mock.check_dcdt_balance(
        &participants[2],
        LAUNCHPAD_TOKEN_ID,
        &rust_biguint!(winning_tickets_user3 * LAUNCHPAD_TOKENS_PER_TICKET),
    ...
    // New participant: 4 tickets, 5 guaranteed, 4 winning ticket
    let winning_tickets_user4 = 4;
    lp_setup
        .b_mock
        .check_rewa_balance(&participants[3], &rust_biguint!(2 * TICKET_COST));
    lp_setup.b_mock.check_dcdt_balance(
...
fn no_unlock_schedule_test() {
    ...
    // Do not set unlock schedule: 100% release immediately
    ...
    assert_eq!(
        nr_winning_tickets as u64,
        winning_tickets_user1 + winning_tickets_user2 + winning_tickets_user3
    );
...
fn no_participants_test() {
    ...
    // Check owner's balances
    let nr_confirmed_tickets = 0u64;
    let expected_owner_rewa = TICKET_COST * nr_confirmed_tickets; // 0
    ...
    lp_setup.b_mock.check_dcdt_balance(
        &lp_setup.owner_address,
        LAUNCHPAD_TOKEN_ID,
        &rust_biguint!(nr_winning_tickets as u64 * LAUNCHPAD_TOKENS_PER_TICKET), // All tokens were refunded to the owner
    );
