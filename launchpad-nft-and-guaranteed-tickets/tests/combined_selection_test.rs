...
fn setup_test() {
    ...
    for (i, p) in part.iter().enumerate() {
        ...
        lp_setup.b_mock.check_rewa_balance(p, &expected_balance);
    }

    lp_setup
        .b_mock
        .check_rewa_balance(lp_setup.lp_wrapper.address_ref(), &sc_balance);
}
...
fn combined_selection_test() {
    ...
    lp_setup
        .b_mock
        .check_rewa_balance(&part[0], &rust_biguint!(120));
    lp_setup
        .b_mock
        .check_rewa_balance(&part[1], &rust_biguint!(30));
    lp_setup
        .b_mock
        .check_rewa_balance(&part[2], &rust_biguint!(120));

    // payment for 2 tickets and an NFT
    lp_setup
        .b_mock
        .check_rewa_balance(&lp_setup.owner_address, &rust_biguint!(120));
    lp_setup
        .b_mock
    ...
    lp_setup
        .b_mock
        .check_dcdt_balance(&part[1], LAUNCHPAD_TOKEN_ID, &rust_biguint!(0));
    lp_setup.b_mock.check_dcdt_balance(
        &part[2],
        LAUNCHPAD_TOKEN_ID,
        &rust_biguint!(LAUNCHPAD_TOKENS_PER_TICKET),
